...
struct AdSizeTests {
    ...
    @Test func standardBannerIsIABStandard() {
        #expect(AdSize.standardBanner.width == 320)
        #expect(AdSize.standardBanner.height == 50)
    }

    @Test func mediumRectangleIsIABStandard() {
        #expect(AdSize.mediumRectangle.width == 300)
        #expect(AdSize.mediumRectangle.height == 250)
    }

    @Test func leaderboardIsIABStandard() {
        #expect(AdSize.leaderboard.width == 728)
        #expect(AdSize.leaderboard.height == 90)
    }

    @Test func customInitPassesBothFieldsThroughUnchanged() {
        let size = AdSize(width: 412, height: 88)
    ...
    @Test func landscapeAnchoredIsShorterThanPortraitAtSameWidth() {
        let width = 600.0
        let portrait = AdSize.portraitAnchored(width: width)
        let landscape = AdSize.landscapeAnchored(width: width)
    ...
    @Test func landscapeAnchoredCompressesTheNarrowAnchorFloor() {
        ...
        let size = AdSize.landscapeAnchored(width: 200)
    ...
    @Test func portraitInlineIsAtLeastAnchoredHeight() {
        let width = 375.0
        let anchored = AdSize.portraitAnchored(width: width)
        let inline = AdSize.portraitInline(width: width, maxHeight: 250)
        #expect(inline.height >= anchored.height)
    }

    @Test func portraitInlineClampsToMaxHeight() {
        let size = AdSize.portraitInline(width: 1000, maxHeight: 100)
    ...
    @Test func portraitInlineNeverDropsBelowAnchoredEvenWithLowMaxHeight() {
        let width = 728.0
        let anchored = AdSize.portraitAnchored(width: width)
        let size = AdSize.portraitInline(width: width, maxHeight: 10)
    ...
    @Test func landscapeInlineIsAtLeastLandscapeAnchoredHeight() {
        let width = 700.0
        let anchored = AdSize.landscapeAnchored(width: width)
        let inline = AdSize.landscapeInline(width: width, maxHeight: 250)
        #expect(inline.height >= anchored.height)
    }

    @Test func landscapeInlineClampsToMaxHeight() {
        let size = AdSize.landscapeInline(width: 1000, maxHeight: 80)
    ...
    @Test func widthIsPassedThroughUnchanged() {
        #expect(AdSize.portraitAnchored(width: 411).width == 411)
        #expect(AdSize.portraitInline(width: 411, maxHeight: 300).width == 411)
    }
}
